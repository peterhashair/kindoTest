...
def get_or_create_booking(db: Session, booking: booking_model.BookingCreate) -> booking_model.BookingDetails:
    existing_booking = db.query(booking_schema.Booking).filter(
...
def _get_booking_by_id(db: Session, booking_id: uuid.UUID) -> booking_schema.Booking:
    booking = db.query(booking_schema.Booking).filter(booking_schema.Booking.id == booking_id).first()
    if not booking:
        raise HTTPException(status_code=404, detail="Booking not found")
    return booking

def get_booking_details(db: Session, booking_id: uuid.UUID) -> booking_model.BookingDetails:
    booking_details = db.query(
     ...
     .filter(booking_schema.Booking.id == booking_id).first()
...
def get_bookings_by_parent_id(db: Session, parent_id: uuid.UUID) -> list[booking_model.BookingDetails]:
    bookings_details = db.query(
        booking_schema.Booking,
        Student.name.label("student_name"),
        Trip.name.label("trip_name"),
        Trip.school_id
     ...
     .join(Trip, booking_schema.Booking.trip_id == Trip.id)\
     .filter(booking_schema.Booking.parent_id == parent_id).all()
...
def update_booking(db: Session, booking_id: uuid.UUID, booking_data: booking_model.BookingUpdate) -> booking_schema.Booking:
