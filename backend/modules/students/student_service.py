...
def create_student(
    db: Session, student: student_model.StudentCreate
    ...
    if len(parents) != len(student.parent_ids):
        raise ExceptionError("One or more parents not found", status_code=404)

    new_student = student_schema.Student(
        name=student.name, gender=student.gender, dob=student.dob
    )
...
) -> List[student_schema.Student]:
    parent = (
        db.query(Parent)
        .filter(Parent.id == parent_id)
        .options(joinedload(Parent.students))
        .first()
    )
    if not parent:
...
def update_student(
    db: Session, student_id: uuid.UUID, student_data: student_model.StudentUpdate
) -> student_schema.Student:
    student = (
        db.query(student_schema.Student)
        .filter(student_schema.Student.id == student_id)
        .first()
    )
    if not student:
